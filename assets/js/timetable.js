...
function loadTimeTable() {
    ...
    fetch('../assets/fetchTimeTable.php', {
        ...
        .then(data => {
            ...
            document.getElementById('timeTableClassSection').innerHTML = "TABLE"  ;
            ...
            if (data['status'] === 'success') {
                ...
                if (data['day'] === 'sunday') {
                    ...
                    const morningDoc   = parser.parseFromString(
                    '<table><tbody>' + data.table1Message + '</tbody></table>',
                    'text/html'
);
                    document.getElementById("lunch-alert").style.display = "block";
                    const afternoonDoc = parser.parseFromString(
                    '<table><tbody>' + data.table2Message + '</tbody></table>',
                    'text/html'
);
                    document.getElementById("saveBtn").disabled = false;
                    document.getElementById("editBtn").disabled = false;
                    const morningRows     = Array.from(morningDoc.querySelectorAll('tr'));
                    const beforeLunchRows = morningRows.slice(0, 4);
                    const spilloverRows   = morningRows.slice(4);

                    const afternoonRows   = Array.from(afternoonDoc.querySelectorAll('tr'));

                    const afterLunchRows = spilloverRows.concat(afternoonRows);

                    const t1 = document.getElementById('timeTable_table1');
                    const t2 = document.getElementById('timeTable_table2');
                    t1.innerHTML = '';  
                    beforeLunchRows.forEach(row => t1.appendChild(row));

                    t2.innerHTML = '';
                    afterLunchRows.forEach(row => t2.appendChild(row));
                }
